                        ...
                        <?php foreach ($roles as $role): ?>
                            <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    <?php
                                    $permissionsList = explode(',', $role['permissions'] ?? '');
                                    foreach ($permissionsList as $permission) {
                                        echo '<span class="inline-flex items-center px-2.5 m-1 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800 mr-1">' . htmlspecialchars(trim($permission)) . '</span>';
                                    }
                                    ?>
                                </td>
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <form action="/admin/roles/edit" method="POST">
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Permissions</label>
                        <div class="space-y-2 max-h-48 overflow-y-auto p-3 border border-gray-300 rounded-md" id="edit_permissions">
                            <?php foreach ($permissions as $permission): ?>
                                <div class="flex items-center">
